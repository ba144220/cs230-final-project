"""
This file is an experiment to see if how much the performance of TableLlama is affected by
1. Making some channels zero (x)
2. Adding random noise to some channels (y)
"""
...
class TableLlamaRotaryEmbedding(torch.nn.Module):
    def __init__(
        ...
        noise_level = self.rope_table_llama["noise_level"]
    ...
    def forward(self, x, position_ids):
        ...
        x_position_ids = torch.zeros_like(position_ids_expanded)
        # Add random noise to y_position_ids
        y_position_ids = position_ids_expanded + torch.randn_like(position_ids_expanded) * self.noise_level
